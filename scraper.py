...
def get_flipkart_live_results(query):
    ...
    try:
        ...
        if resp.status_code == 200:
            soup = BeautifulSoup(resp.content, "html.parser")
            # Flipkart current DOM classes for mobile/electronics
            cards = soup.select("div._1AtVbE, div.tUxRFH, div._75nlfW, div._1sdMkc")
            
            for card in cards[:4]:
                title_elem = card.select_one("div.KzDlHZ, div._4rR01T, a.wjcEIp, div.header")
                price_elem = card.select_one("div.Nx9bqj, div._30jeq3")
                link_elem = card.select_one("a.CGtC5Q, a._1fQZEK, a.VJA3rP, a[href*='/p/']")
